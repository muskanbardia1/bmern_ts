...
import { loadUser, logout, editUser } from "../flux/actions/authActions";
import { getUserById } from "../flux/actions/userListActions";
...
const UserDashboard: React.FC<IUserDashboard> = ({
  ...
  useEffect(() => {
    loadUser();
    setData({
      fname: auth.isLoading || !auth.user ? "" : auth.user.fname,
      lname: auth.isLoading || !auth.user ? "" : auth.user.lname,
      email: auth.isLoading || !auth.user ? "" : auth.user.email,
      address: auth.isLoading || !auth.user ? "" : auth.user.address,
      phone: auth.isLoading || !auth.user ? "" : auth.user.phone,
      image: auth.isLoading || !auth.user ? "" : auth.user.image,
      password: "",
      password2: "",
    });
  }, [loadUser,  auth.user]);
