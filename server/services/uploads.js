...
const AddDocumentfileFilter = async (req, file, cb) => {
  ...
  if (ext.indexOf(file.mimetype.toLowerCase()) >= 0) {
    if (req.body.documentName && req.body.sharedWith) {
      ...
      if (
        req.body.sharedWithOrganisation &&
        typeof req.body.sharedWithOrganisation == "string"
      ) {
        try {
          req.body.sharedWithOrganisation = JSON.parse(
            req.body.sharedWithOrganisation
          );
        } catch (error) {
    ...
    cb(new Error("Please upload document in valid format"));
  }
};
